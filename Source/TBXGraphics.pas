...
{$IF NOT DEFINED(JR_D6) OR (CompilerVersion < 33)}
...
{ Color effects }
  ...
  TColorEffectItem = record
    ID: Integer;
    Param: Cardinal;
    Proc: TColorEffectProc;
  end;
  TColorEffectItemDynArray = array of TColorEffectItem;
...
var
  DIV_TABLE: array [Byte, Byte] of Byte;           // [$00..$FF]
  ALPHAMUL_TABLE: array [Byte, Byte] of Cardinal;  // [$00000000..$FF000000]

type
  PDivTableLine = ^TDivTableLine;
  TDivTableLine = array [Byte] of Byte;
...
begin
  ...
  begin
    ...
    begin
      ...
      Inc(PByte(SrcLine), SizeOf(TRGBQuad));
    end;
    ...
    Inc(PByte(DstLine), DstData.RowStride shl 2);
  end;
end;
...
var
  ...
  V, VT, VB, AL, RL, GL, BL, AR, RR, GR, BR: Cardinal;
begin
  ...
  begin
    ...
    if MapVert = nil then Inc(PByte(SrcLine), SrcData.RowStride shl 2);
    if Blend then BlendPixels_ATOP_LL(Buffer[0], DstLine[0], ClipW, $FF);
    Inc(PByte(DstLine), DstData.RowStride shl 2);
  end;
...
begin
  ...
  FBitmapInfo.bmiHeader.biSize := SizeOf(TBitmapInfo);
...
begin
  ...
  ImageIndex := Index and $FFFF;
  OverlayIndex := Index shr 16 and $0F;
